...
- **트리거 문장**
  ...
  - "부분 문제로 쪼개짐", "같은 계산 반복"
- **핵심 불변식**
  - `dp[state]`의 의미를 고정하면 점화식으로 전체가 일관되게 계산된다
  - 작은 상태의 정답이 큰 상태 정답의 재료가 된다
- **실수 포인트**
  - 상태 정의 모호함 / 기저 조건 누락 / 순회 순서 오류 / 답 위치 착각
- **템플릿 위치**
  - 이 문서의 `Top-Down`, `Bottom-Up` 코드 블록
...
## 📚 사전 지식 (DP를 풀기 전에 알면 좋은 것)

### 필수 선행 개념

**1. 재귀(Recursion)를 자유롭게 쓸 수 있는가?**

DP의 본질은 "재귀 + 중복 제거"다. Top-Down DP는 재귀에 메모이제이션을 붙인 것이고, Bottom-Up DP는 그 재귀를 뒤집어 반복문으로 바꾼 것이다. 재귀로 문제를 분해하는 감각이 없으면 점화식 자체가 와닿지 않는다.

**2. 수학적 귀납법의 사고방식**

> "n-1까지 답이 구해져 있다고 **가정**하면, n의 답은 어떻게 구할 수 있는가?"

DP 점화식을 세우는 과정이 정확히 이것이다. 예를 들어 동전 문제에서:

> "j-coin원을 만드는 방법의 수를 **이미 안다고 가정**하면, j원은?"

### DP를 관통하는 3가지 질문

모든 DP 문제는 결국 이 세 질문에 답하는 것이다:

| 순서 | 질문 | 예시 (동전 1) | 예시 (배낭) |
|---|---|---|---|
| **① State** | dp[?]가 무엇을 의미하는가? | dp[j] = 금액 j를 만드는 경우의 수 | dp[w] = 무게 w에서의 최대 가치 |
| **② Transition** | dp[?]를 어떻게 채우는가? | dp[j] += dp[j-coin] | dp[w] = max(dp[w], dp[w-w_i]+v_i) |
| **③ Base case** | 시작점은 무엇인가? | dp[0] = 1 (빈 조합 1가지) | dp[0] = 0 (아무것도 안 넣음) |

이 중 가장 어려운 것이 **① State 정의**다. 나머지 둘은 State가 정해지면 자연스럽게 따라온다.

### 연습 순서 제안

```
① 1D DP (피보나치, 계단 오르기)
   → State 정의와 점화식의 감을 잡는 단계
