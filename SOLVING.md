...
## 디버깅 및 실수 방지 전략 (프로그래머스/코테 환경)

디버거가 없는 환경에서 오타와 버그를 줄이는 필승 전략입니다.

### 1. 🛑 오타 원천 봉쇄 네이밍

창의적인 변수명은 오타의 주범입니다. **기계적인 네이밍 규칙**을 정해두세요.

| 상황 | 변수명 규칙 | 이유 |
|---|---|---|
| **좌표 (2차원)** | `r, c` (row, col) 또는 `y, x` | `i, j`는 헷갈림. `x, y`는 수학과 배열 순서가 반대라 위험. `r, c`가 가장 직관적. |
| **방향 벡터** | `dr, dc` / `dy, dx` | `d` 접두어로 통일. `dir` 리스트는 `DIR` 또는 `d`로 짧게. |
| **정답** | `answer` / `result` / `ans` | 고민하지 말고 하나만 정해서 쓰기. |
| **임시 변수** | `tmp` (절대 쓰지 않기) | `next_pos`, `new_val` 처럼 의미 부여. `num`도 너무 흔함. |
| **카운트/합** | `cnt`, `total` | `count`도 길다. `cnt`로 통일. |
| **길이/크기** | `N, M` | `len(grid)` 등을 변수 `N`에 담아두고 쓰기 (오타 방지 + 가독성). |

**꿀팁**: `len(board)`, `len(board[0])` 처럼 함수 호출을 반복하지 말고 초반에 `H, W = len(park), len(park[0])` 처럼 변수에 담으세요. 오타 확률이 확 줄어듭니다.

### 2. 🖨️ "프린트 디버깅"의 기술

디버거 없이 `print()`로 확인하는 법입니다.

1.  **진입점 확인**: 루프/재귀 시작 부분에 인자를 찍으세요.
    ```python
    def dfs(cur, depth):
        print(f"In: {cur}, Depth: {depth}") # 주석 처리하기 쉽게
    ```
2.  **경계값 확인**: `if`문 조건이 의심되면 조건을 `print` 해보세요.
    ```python
    # 의심: if 0 <= ny < H and ...
    print(ny, H, 0 <= ny < H) # True/False 눈으로 확인
    ```
3.  **단계별 상태**: 상태가 변하는(갱신되는) 직후에 찍으세요.
    ```python
    print(f"Before: {grid[r][c]}")
    grid[r][c] = 1
    print(f"After: {grid[r][c]}")
    ```

### 3. 🛡️ 방어적 코딩 (Assert)

로직이 복잡하면 나만의 **검증 코드**를 심으세요.

```python
# 이동 후 좌표가 맵 밖으로 튀지 않는지 불안하다면?
if not (0 <= ny < N):
    print(f"ERROR: Out of bound {ny}, {nx}") # 에러 로그 직접 출력
```

### 4. 🧩 점진적 구현 (Incremental Coding)

**절대 50줄을 한 번에 짜고 실행하지 마세요.**

1.  **입력 파싱 확인**: 입력을 잘 받았는지 `print`로 확인.
2.  **1단계 로직 확인**: 예를 들어 "이동"만 먼저 구현하고 잘 움직이는지 확인.
3.  **2단계 로직 추가**: "장애물 검사" 추가 후 확인.

작게 쪼개서 실행하면 버그의 위치가 방금 짠 5줄 안에 갇힙니다.
